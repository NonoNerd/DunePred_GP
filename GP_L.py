...
from MDAL.mda import MaxDiss_Simplified_NoThreshold
from MDAL.plots.mda import Plot_MDA_Data
...
namesOut = ['L']
namesAll=['Depth','Width','d50','ust','ws','Lsat_b','Lsat_s','L'] 
...
Y_train=(pd.DataFrame(data=train, columns=namesOut))
X_test = scale.transform(pd.DataFrame(data=test, columns=namesIn))
Y_test=(pd.DataFrame(data=test, columns=namesOut))
...
#Define Algorithm with hyperparameters
est_gp = SymbolicRegressor(population_size=5000,
                           generations=1000, init_depth=(4, 20), stopping_criteria=0.0,
                           function_set=function_set, metric='rmse',
                           p_crossover=0.7, p_subtree_mutation=0.1,
                           p_hoist_mutation=0.05, p_point_mutation=0.1,
                           max_samples=1, verbose=1,parsimony_coefficient=0.0003, random_state=0,n_jobs=1)
...
fig=plt.figure(1)
plt.plot(Y_train, y_train_predicted, '*k')
plt.text(0.0, 5, 'RMSE :'+'{:.2f}'.format(mean_squared_error(y_train_predicted,Y_train,squared=False)), style='italic', bbox={'facecolor': 'grey', 'alpha': 0.5, 'pad': 6})
plt.text(0.0, 4.2, 'R2 :'+'{:.2f}'.format(est_gp.score(X_train,Y_train)), style='italic', bbox={'facecolor': 'grey', 'alpha': 0.5, 'pad': 6})
# plt.xlim(0, 1.6), plt.ylim(0, 1.6)
plt.xlabel ('L Lab (Train Set)'); plt.ylabel ('L model (Train Set)')
# best fit line (1:1)
plt.plot([0, 6], [0, 6], '--r')
...
plt.scatter(np.array(Yr)[0:5,0],pred_r,color='g',marker=(5, 1),s=20, alpha=0.8)
plt.text(0.0, 5, 'RMSE :'+'{:.2f}'.format(mean_squared_error(Y_test,y_test_predicted,squared=False)), style='italic', bbox={'facecolor': 'grey', 'alpha': 0.5, 'pad': 6})
plt.text(0.0, 4.2, 'R2 :'+'{:.2f}'.format(est_gp.score(X_test,Y_test)), style='italic', bbox={'facecolor': 'grey', 'alpha': 0.5, 'pad': 6})
# plt.xlim(0, 1.6), plt.ylim(0, 1.6)
plt.xlabel ('L Lab (Test Set)'); plt.ylabel ('L model (Test Set)')
plt.rc('grid', linestyle="dotted", color='black')
plt.grid()
# best fit line (1:1)
plt.plot([0, 6], [0, 6], '--r')
...
fig=plt.figure(3)
plt.plot(Ya, y_predicted, '*k')
plt.text(0.0, 5, 'RMSE :'+'{:.2f}'.format(mean_squared_error(Ya,y_predicted,squared=False)), style='italic', bbox={'facecolor': 'grey', 'alpha': 0.5, 'pad': 6})
plt.text(0.0, 4.2, 'R2 :'+'{:.2f}'.format(est_gp.score(Xtn,Ya)), style='italic', bbox={'facecolor': 'grey', 'alpha': 0.5, 'pad': 6})
# plt.xlim(0, 1.6), plt.ylim(0, 1.6)
plt.xlabel ('L Lab (All data)'); plt.ylabel ('L model (All data)')
plt.rc('grid', linestyle="dotted", color='black')
plt.grid()
# best fit line (1:1)
plt.plot([0, 6], [0, 6], '--r')
